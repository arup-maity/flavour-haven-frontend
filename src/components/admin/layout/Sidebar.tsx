...
const Sidebar: React.FC<PropsType> = ({ menuCollapsed, menuHover }) => {
  ...
  return (
    <div className="sidebar-menu-content w-full h-[calc(100%-70px)] border-r py-1">
...
const NavItem = (props: any) => {
  ...
  return (
    <li className="webx-menu-item">
      ...
      >
        <span className="menu-icon flex items-center transition-all duration-300 ease-in-out">
          {item.icon}
        </span>
...
export const NavItemGroup = (props: any) => {
  ...
  return (
    <li
      className={clsx(
        "webx-menu-item webx-has-submenu",
        {
    ...
    >
      ...
      >
        ...
        <span className="flex-grow text-base font-medium line-clamp-1">
          {item.title}
        </span>
        <span
          className={clsx(
            `flex items-center text-gray-400 transition-transform delay-150 ease-in-out`,
            {
              "rotate-90":
                groupActive?.includes(item.id) ||
                groupOpen?.includes(item.id) ||
      ...
      <ul
        className={clsx(
          `grid grid-rows-[0fr] transition-[grid-template-rows] duration-500 ease-in-out ps-4`,
          {
